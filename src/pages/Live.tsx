...
import { Search, Loader2, ChevronLeft, ChevronRight, Tv } from 'lucide-react';
...
const Live = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const urls = await getTVPlaylists();
  ...
  const totalPages = Math.ceil(filtered.length / CHANNELS_PER_PAGE);
  const paginated = filtered.slice((page - 1) * CHANNELS_PER_PAGE, page * CHANNELS_PER_PAGE);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">{filtered.length} channels available</p>
          </div>
